...
def Alignment_error_rate(gt_location, res_location):
	...
	if len(gt_location) != len(res_location):
		...
		for loc_gt, loc_res in zip(gt_location, res_location):
			...
			if loc_gt[0]<=loc_res[0]<=loc_gt[1] or loc_gt[0]<=loc_res[1]<=loc_gt[1]:
				match = match + 1
			else:
				match = match
...
# ------------------------------ get word location of ground truth  --------------------------
	...
	AER = Alignment_error_rate(gt_location,res_location)
	AR = Alignment_recall(gt_location,res_location)
	AP = Alignment_precision(gt_location,res_location)
