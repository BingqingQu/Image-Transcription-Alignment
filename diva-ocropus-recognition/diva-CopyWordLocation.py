...
def Renanme(page_image, GT_path, Des_path):
    ...
    txt_files = [os.path.join(GT_path, item) for item in sorted(os.listdir(GT_path))]
    ...
    for i, item in enumerate(txt_content):
        ...
        if i+1 < 10:
            dst_txt = os.path.join(Des_path, '{}{}.lc.txt'.format('0', i + 1))
        else:
            dst_txt = os.path.join(Des_path, '{}.lc.txt'.format(i + 1))
